...
4. 觸發（GMT+8，marker 去重一天只寄一次）：由 Cloudflare Worker `digest-cron` 準點 `workflow_dispatch` — 08:00 主班、12:00 補寄、14:00 最後一班（沒稿才寄警示信）；每週回顧週六同班次一併寄出。GitHub 自家 schedule 已移除（延遲久、會整班丟失）。改時間見 `~/Workspace/digest-cron`。
